...
public class ValidateCodeAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		if (user == null) {
			...
			if (user.getAttempts() >= AppSite.me().getAttempts()) {
				...
				if (minute < AppSite.me().getAttemptsWaitTime()) {
					throw new AuthenticationServiceException("密码错误超过" + AppSite.me().getAttempts() + "次，账号已被锁定"
							+ AppSite.me().getAttemptsWaitTime() + "分钟");
				} else {
					user.setAttempts(0);
				}
			}
