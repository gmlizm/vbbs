...
public class TopicController extends BaseController {
  ...
  public String create() throws Exception {
    ...
    if (user.getBlock()) throw new Exception("你的帐户已经被禁用了，不能进行此项操作");

    if (user.getScore() + AppSite.me().getCreateTopicScore() < 0) throw new Exception("你的积分不足，不能发布话题");
    ...
    if (AppSite.me().getMaxCreateTopic() < topicService.countByInTimeBetween(date1, date2))
  ...
  public String save(Integer nodeId, String title, String url, String content,
    ...
    if (StringUtils.isEmpty(title)) {
      ...
      // update score
      user.setScore(user.getScore() + AppSite.me().getCreateTopicScore());
      ...
      scoreLog.setChangeScore(AppSite.me().getCreateTopicScore());
      ...
      String des = freemarkerUtil.format(AppSite.me().getScoreTemplate().get(ScoreEventEnum.CREATE_TOPIC.getName()), params);
  ...
  public String update(@PathVariable Integer id, Integer nodeId, String title, String url, String content,
    ...
    Node oldNode = nodeService.findById(topic.getNodeId());
    ...
    //更新node的话题数
    if (!Objects.equals(oldNode.getId(), nodeId)) {
      nodeService.dealTopicCount(oldNode, -1);
      nodeService.dealTopicCount(node, 1);
    }
